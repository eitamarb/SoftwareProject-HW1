...
Data *handle_stream(const char *file_name) {
    ...
    fclose(file);
    N = counter;
    matrix = list_to_matrix(head, N, vector_size);
...
int main(int argc, char *argv[]) {
    ...
    while (iterations > 0 && !valid) { /* line 67 on python file */
        ...
        valid = 1;
        cluster_members_counter = (int *) calloc(K, sizeof(int)); /* allocate memory for the c_m_c array */
        cluster_members_counter_copy = (int *) calloc(K, sizeof(int)); /* allocate memory for the c_m_c_c array */
